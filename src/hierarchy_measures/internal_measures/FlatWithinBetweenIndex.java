...
public class FlatWithinBetweenIndex extends CommonQualityMeasure {
	...
	private static final Logger log = LogManager.getLogger(FlatWithinBetweenIndex.class);
	...
	public double getMeasure(Hierarchy h) {
			...
			if (!nodes[n1].getNodeInstances().isEmpty()) {
				for (int i1 = 0; i1 < n1Instances.size(); i1++) {
					for (int i2 = i1; i2 < n1Instances.size(); i2++) {
						double distance = dist.getDistance(n1Instances.get(i1), n1Instances.get(i2));// TODO: random
																										// index access
																										// is slow on
																										// linked lists
				...
				for (int n2 = n1 + 1; n2 < nodes.length; n2++) {
					if (!nodes[n2].getNodeInstances().isEmpty()) {
						double distance = dist.getDistance(nodes[n1].getNodeRepresentation(),
		...
		if (maxWithinNodeDistance == (-1) * Double.MAX_VALUE) {
			log.error("FlatWithinBetweenIndex.getMeasure maxWithinNodeDistance didn't change! It is probably "
					+ "because every cluster contain at maximum 1 instance. Returning NaN.");
			return Double.NaN;
		}
		if (minNodesCentersDistance == Double.MAX_VALUE) {
			log.error("FlatWithinBetweenIndex.getMeasure minNodesCentersDistance haven't changed, so there "
