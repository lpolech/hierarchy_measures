...
public class FlatDunn2 extends CommonQualityMeasure {
	...
	private static final Logger log = LogManager.getLogger(FlatDunn2.class);
	...
	protected double calculate(Hierarchy h, boolean returnMeasureReversion) {
		...
		if (minDistanceBetweenPointsInDifferentClusters == Double.MAX_VALUE) {
			log.error("FlatDunn2.getMeasure minDistanceBetweenPointsInDifferentClusters didn't change! It is "
		...
		if (maxDistanceBetweenPointsWithinCluster == (-1) * Double.MAX_VALUE) {
			log.error("FlatDunn2.getMeasure maxDistanceBetweenPointsWithinCluster didn't change! It is probably "
